...
| 「＋」(`.main-tabs-new`) | タブの数が変わったとき・グループを畳む / 開くとき (最後のタブのすぐ右、グループがあれば最後のグループのタブのすぐ右。`ui-surface.md` のタブの決まり) |
| タブのグループの札 (`.main-tab-group`) とグループの線・畳んだグループより左のタブ | 自分より左のタブ・グループの数・幅が変わったとき (タブを開く・閉じる・左のグループを畳む / 開く・グループの並びが変わる)。自分のグループを畳む / 開くでは左端が動かない (右が枚数の分だけ伸びる。タブを縮める割合は枚数を除き、畳んだタブも数えて決める: `fitTabs`)。プロジェクトを切り替えても (同じタブの列なので) 動かない。列があふれて前面のタブを見せるための横送りでは動く |
...
**既知の残り (最初の描画)。**
- 2 面かどうかは保存したタブを読むまで分からないので、控え (`views/shell/early-look.ts` の `split`。
  タブの配置と同じく全プロジェクト共通の 1 つ) で数える。早いスクリプトも、タブを読み戻すまでの app.ts
  (`BOOT_SPLIT`) も同じ控えで 2 面の本文の幅 (961) を残す (1 面で数えていた間は、2 面の一覧の画面を
  読み込むたびに列が詰めた幅へ動き、CLS 0.16〜0.25 あった)。控えが無い初めての読み込みと、別の窓で
  面の数を変えた直後だけ 1 面 / 2 面を数え違えて動く
...
- 保存したタブが URL と違う画面を前面にする読み込み (`urlKeepsSavedFront`) では、URL から付けた
  印と違う並びになり、そのときだけ動く
- URL の `?terminal=` にシェルがあれば前面は端末とみなし、一覧を出さない (app.ts の
  `syncTerminalFromUrl` が開く。見ないと、端末が前面のまま読み込み直すたびに背面の画面の一覧を出して
  から外し、CLS 0.22 動いた)。タブを読み戻すまでの app.ts も同じ (`RESTORING_TABS`)。どちらの面の
  端末かは分からないので、2 面で右の面の端末にフォーカスがあるときは、左の面の一覧を出さずに描き
  始めて動く
