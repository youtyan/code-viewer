...
**既知の残り (読み込み中→後)。**
- Diff のカードの最後のハンクの後の「下へ広げる」行 (36px) は、描いた後にファイルの残りを
  問い合わせてから足す (`views/hunk-expand.ts` の `probeAndAttachTrailingExpandControls`) ので、
  見積もりに入らず、足したときに下のカードが下がる。削除したファイルではこの問い合わせが
  失敗し、エラーの行 (52〜74px) が足される
