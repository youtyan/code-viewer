...
| メインの面のターミナルのタブ | 箱は `views/terminal/terminal-view.ts` の `tabElement` (`#terminal-tab`)。前面のタブがターミナルのときだけ `body.main-terminal-front` で出す。タブの名前はエージェントを映していれば「種類 · 状態」(`agentsText().kind` / `.state`)、絵は状態の印 (`.terminal-mark-*`)。下パネルの一覧の行の「タブで開く」は `terminal-row-tab` (タブで表示中なら同じボタンが印) |
