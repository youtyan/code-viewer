...
- **横に並べる (2 面)** の入口は 4 つ。タブの右クリックの「右に分割」・Alt＋クリック・タブを右端の帯
  (`.main-split-drop`。ドラッグ中だけ本文の右半分に出し、右の列は覆わない) へドラッグ・タブ列の分割のボタン。
...
- 2 面のとき、開く面は**フォーカスのある面** (画面のタブは左の面だけ)。フォーカスは面の中を押す・
  タブを押すで移り、フォーカスのある面の前面のタブに上端の線 (`.main-tab-focused`) が付く
...
- **PWA (インストールした窓)** ではブラウザのタブのキーがアプリのタブに効く: ⌘/Ctrl+W で閉じる・
  ⌘/Ctrl+T で＋のメニュー・⌘/Ctrl+1〜9 で n 番目・Ctrl+Tab / Ctrl+Shift+Tab で次 / 前・
  ⌘/Ctrl+Shift+T で最後に閉じたタブを開き直す。振り向けは `core/pwa.ts` で、中身は上と同じモデルの操作
  (`close` / `nextTab` / `prevTab` / `activateIndex` / `reopenClosed`) を呼ぶだけ。ブラウザのタブの中では変えない
- **タブ列のキー**: Tab で列に入る (前面のタブだけ `tabindex=0`)・←→ Home End でタブを移る・
  Enter / Space で前面に・Delete で閉じる・Ctrl+Shift+PageUp / PageDown で並べ替え
...
- 右クリックの項目名は英日で同じ語を使う: 新しいタブで開く / Open in new tab、右に分割して開く /
  Open to the right、開いたままにする / Keep open、右に分割 / Split right

入口ごとの例外 (表に無い入口は上の表のとおり):

| 入口 | 例外 |
|---|---|
| 画面 (Diff・History・作業ツリー・Data・Search・Tools・Help・全体ボード) | 画面のタブは種類ごとに 1 つで常に固定、左の面だけ。どの押し方でもそのタブを前面に出す |
...
| ターミナル (エージェントの行・パレットのセッション・＋) | 常に固定。1 つのシェルは 1 か所にしか置けない (あれば前面に出す)。Alt は反対の面 |
| 端末に出た画像のリンク・画像の棚 | 画像のタブ (仮) を反対の面に開く (端末を隠さない)。Alt は大きく開く覆い。⌘/Ctrl/Shift は端末の選択に任せる |
