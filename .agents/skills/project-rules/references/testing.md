...
| サーバを起こすテストは `--standalone` を付ける | 既定の `code-viewer` は入口のサーバ（`entry.json` を見て、動いている入口があれば委ねて終わる）。入口そのものを確かめるテスト（`entry-server.test.ts`）は、状態ディレクトリ・登録簿・`TMUX_TMPDIR` をテストごとの一時ディレクトリにし、`TMUX`・`TMUX_PANE` を外す（入口は tmux を巡回する） |
