...
import { enhanceMediaCard, isAudio, isImage, isVideo } from "./media-embed";
import {
  type AppRoute,
  buildRawFileUrl,
  buildRoute,
...
(() => {
  ...
  function isRepoSidebarReusable(ref: string): boolean {
    return (
      REPO_SIDEBAR_REF === (ref || "worktree") && isRepositorySidebarMode()
    );
  }
  ...
  function askNewDirectoryName(
    ...
    return new Promise((resolve) => {
      ...
      const backdrop = createTrashDialog(
        "New Folder",
        `Create a folder in "${path || PROJECT_NAME || "repository"}".`,
        [cancel, create],
      );
  ...
  async function moveRepoPathToTrash(path: string) {
    ...
    if (body.undo) UNDO_STACK.unshift(body.undo);
    return true;
  }
  ...
  function handleSidebarContextMenu(event: MouseEvent) {
    ...
    if (!entry) return;
    if (
      showRepoContextMenu(event, entry, REPO_SIDEBAR_REF || "worktree", () =>
        loadRepo(),
      )
    )
  ...
  type VirtualSourcePagingKeyboardEvent = KeyboardEvent & {
