...
(() => {
  ...
  const CLIENT_SCOPE_OMIT_DIRS_DEFAULT = [
    'node_modules', '.venv', 'venv', '.next', '.nuxt', '.svelte-kit', '.astro', '.vercel',
    'dist', 'build', 'out', 'target', '.gradle', '__pycache__', '.pytest_cache', '.tox',
    '.terraform', '.idea', '.vscode', 'vendor', '.cache', 'coverage', 'DerivedData',
    'Pods', 'bin', 'obj',
  ];
  ...
  function normalizeScopeOmitDirs(value: string[] | string): string[] {
    ...
    return [...new Set(raw
      .map(item => item.trim())
      .filter(item => item && item.length <= 64 && !item.includes('/') && !item.includes('\\') && item !== '.' && item !== '..' && item !== '.git'))]
      .slice(0, 100)
  ...
  function appendScopeOmitDirsParam(params: URLSearchParams) {
    ...
    if (saved != null) params.set('omit_dirs', saved.join(','));
  }
  ...
  function syncSidebarHeaderHeight() {
    requestAnimationFrame(() => {
      ...
      if (head) document.documentElement.style.setProperty('--sidebar-head-h', Math.ceil(head.getBoundingClientRect().height) + 'px');
    });
  }
  ...
  function repoFileCacheKey(ref: string): string {
    const saved = savedScopeOmitDirs();
    return `${ref}\0${saved ? saved.join('\0') : 'server'}`;
  }
  ...
  function setSidebarTreeActionIcons() {
    ...
    if (sidebarToggle) sidebarToggle.innerHTML = iconSvg('octicon-sidebar', STATE.sidebarHidden ? SIDEBAR_SHOW_16_PATHS : SIDEBAR_HIDE_16_PATHS);
    if (expand) expand.innerHTML = iconSvg('octicon-chevron-down', EXPAND_ALL_16_PATHS);
    if (collapse) collapse.innerHTML = iconSvg('octicon-chevron-up', COLLAPSE_ALL_16_PATHS);
  }
  ...
  function placeSidebarToggle() {
    ...
    const toolbar = document.querySelector<HTMLElement>('.gdp-repo-toolbar, .gdp-file-detail-header');
    const restoreHost = toolbar ||
  ...
  function scopeOmitSourceLabel(): string {
    return savedScopeOmitDirs() != null
      ? 'Browser override'
      : 'Server default';
  }
  ...
  async function openScopeSettings() {
    ...
    const input = document.querySelector<HTMLTextAreaElement>('#scope-omit-dirs');
    const sidebarFontSize = document.querySelector<HTMLSelectElement>('#sidebar-font-size');
    const codeFontSize = document.querySelector<HTMLSelectElement>('#code-font-size');
    const source = document.querySelector<HTMLElement>('#scope-omit-source');
    if (!pop || !input || !sidebarFontSize || !codeFontSize || !source) return;
    await loadSettings();
  ...
  function saveScopeSettings() {
    ...
    const sidebarFontSize = document.querySelector<HTMLSelectElement>('#sidebar-font-size');
    const codeFontSize = document.querySelector<HTMLSelectElement>('#code-font-size');
    if (!input || !sidebarFontSize || !codeFontSize) return;
    ...
    for (const item of items) {
      if (item.kind === 'dir') {
        ...
        if (dir.children_omitted) {
          ...
          li.title = dir.children_omitted_reason === 'heavy'
        ...
        if (dir.children_omitted) {
          ...
          omitted.title = dir.children_omitted_reason === 'heavy'
        ...
        if (onFileClick) {
          li.addEventListener('click', (e) => {
            e.stopPropagation();
            if (dir.children_omitted_reason === 'internal' || dir.children_omitted_reason === 'truncated') return;
            onFileClick({
              path: dir.path,
  ...
  function hydrateRefSelectorMounts() {
    document.querySelectorAll<HTMLElement>('[data-ref-selector-mount]').forEach(mount => {
  ...
  function syncHeaderMenu() {
    document.querySelectorAll<HTMLAnchorElement>('.app-menu-item, .global-help-link').forEach(link => {
  ...
  function wireRefSelectorInput(input: HTMLInputElement, onPick?: (ref: string) => void) {
    ...
    if (onPick) input.addEventListener('change', () => onPick(input.value || 'worktree'));
  }
  ...
  const refFromInput = $<HTMLInputElement>('#ref-from');
  const refToInput = $<HTMLInputElement>('#ref-to');
  ...
  wireRefSelectorInput($<HTMLInputElement>('#repo-target'), (ref) => {
    if (STATE.route.screen !== 'file') return;
