        ...
        groups: [
          ...
          },
          {
            ...
            blocks: [
              ...
              {
                kind: "paragraph",
                text: "The row of tabs at the top keeps files and screens open; there is no header row above it. The project name, its branch and six view icons (Files, Diff, History, Worktrees, Data, Work log; hover for the name and key) stay fixed at the head of the right column (on the tab row and just below it); the right column also holds the Files tree and the lists of History and Worktrees, while the left sidebar holds the projects and agents. Switching tabs or screens never moves them; only when you fold the right column do they move to the left end of the tab row. A file opened with one click (tree, palette, a line link, View File) opens in a preview tab with an italic name, and the next file replaces it; double-click the tree row or the tab, or choose Keep open, to keep it. Diff, History, Worktrees, Data and Work log each have one tab: their icon brings that tab back as you left it. Files is not a tab: the folder view is what the left side shows when no tab is selected (the Files icon, g r, or a folder in the tree clears the selection), and closing the last tab shows it too. Right-click a tab for Close / Close others / Close to the right / Keep open / Copy path, drag tabs to reorder, and use g t / g T (next / previous), g x (close) and g 1–9. The tabs are remembered per project, except the ones that do not belong to a project: the agent board, Tools, Settings & Help, terminals and images shown by a terminal stay open when you switch projects. Screens (Diff, History, Worktrees, Data, Work log, Agents, Settings & Help) have one place to be drawn, so they stay on the left side; the right side holds files, terminals and images. Each side draws its own file, so two files, or the same file twice, sit side by side, each with its own scroll, line selection and Code / Preview / Blame view; a file's History opens on the left. The split button at the right of the row (or Split right on a file, terminal or image tab, or dragging one onto the right half) shows two sides; drag the line between them to resize, drag files, terminals and images between the sides or use Move to other side, and press g o to move to the other side. The tree and the palette open a file on the side that has the focus; Alt+click a file in the tree to open it on the other side (with one side, it splits and opens it on the right). The URL follows the focused side: a file on the right adds pane=right, so a reload and Back / Forward come back to it. Image files open in an image tab with zoom, previous / next, Copy path and Open folder. The buttons that used to sit at the right of the header row (annotations, Copy AI context, auto update, cancel requests, theme, repository page) are at the right of the bottom bar.",
              },
              {
                kind: "paragraph",
        ...
        groups: [
          ...
          },
          {
            ...
            blocks: [
              ...
              {
                kind: "paragraph",
                text: "最上段のタブ列に、開いたファイルと画面が並びます (その上に見出しの行はありません)。プロジェクト名・ブランチと 6 つの画面の絵柄 (Files・Diff・History・Worktrees・Data・Work log。カーソルを置くと名前とキー) は右の列の頭 (タブ列の行とその下) に固定されます。右の列には Files の木と History・Worktrees の一覧もあり、左のサイドバーにはプロジェクトとエージェントが並びます。タブや画面を切り替えても動かず、右の列を畳んだときだけタブ列の左端に移ります。1 回押して開いたファイル (木・パレット・行リンク・View File) は名前が斜体の仮のタブで、次に開いたファイルに置き換わります。木の行かタブをダブルクリックするか、「開いたままにする」で固定します。Diff・History・Worktrees・Data・Work log はそれぞれタブが 1 つで、絵柄を押すとそのタブが前に見ていた状態で前面に出ます。Files はタブではありません。フォルダ表示は、左の面でタブを選んでいないときに出る既定の本文です (Files の絵柄・g r・木のフォルダで選択が外れます。最後のタブを閉じたときもこれが出ます)。タブの右クリックで閉じる・ほかを閉じる・右側を閉じる・開いたままにする・パスをコピー、ドラッグで並べ替え、g t / g T (次 / 前)、g x (閉じる)、g 1〜9 で移れます。タブの並びはプロジェクトごとに覚えます。ただしプロジェクトに属さないタブ (エージェントの全体ボード・Tools・設定とヘルプ・ターミナル・ターミナルに出た画像) は、プロジェクトを切り替えても開いたまま残ります。画面 (Diff・History・Worktrees・Data・Work log・エージェント・設定とヘルプ) は描く場所が 1 つなので左の面にだけ置き、右の面にはファイル・ターミナル・画像を置きます。ファイルは面ごとに描くので、2 つのファイルや同じファイルを左右に並べられ、スクロール・行の選択・Code / Preview / Blame の切り替えは面ごとです (ファイルの History は左の面で開きます)。タブ列の右端の分割ボタン (ファイル・ターミナル・画像のタブの「右に分割」、それを右半分へドラッグでも) で左右 2 面になり、間の線をドラッグで幅を変え、ファイル・ターミナル・画像は面の間でドラッグするか「反対側へ移す」で移し、g o でもう一方の面へ移ります。木とパレットはフォーカスのある面にファイルを開き、木のファイルを Alt+クリックすると反対の面に開きます (1 面なら右に分けて右に開きます)。URL はフォーカスのある面に合わせ、右の面のファイルなら pane=right が付くので、再読み込みや戻る・進むでその面に戻ります。画像のファイルは、倍率・前後・パスのコピー・フォルダを開くが付いた画像のタブで開きます。以前の見出しの行の右端にあったボタン (注釈・AI 用コンテキストのコピー・自動更新・通信の中止・テーマ・リポジトリのページ) は最下段の右にあります。",
              },
              {
                kind: "paragraph",
