        ...
        groups: [
          ...
          },
          {
            ...
            blocks: [
              ...
              {
                kind: "paragraph",
                text: "In large repositories the file list loads folder children on demand. Folders you open are remembered and automatically re-expanded after a reload.",
              },
              {
                kind: "paragraph",
              ...
              {
                kind: "paragraph",
                text: "Tab reaches the tree once, on the selected row (or the first row). There, ↑ / ↓ move, Home / End jump to the first / last row, → opens a folder, ← folds it or goes to its parent folder, and Enter opens the row; j / k / l / h keep working as before. Shift+L moves the focus to the main area, and Shift+H brings it back to the file list (on Diff, History and Worktrees, to the list of that screen).",
              },
              {
                kind: "paragraph",
          ...
          },
          {
            ...
            blocks: [
              ...
              {
                kind: "paragraph",
                text: "The file list filter takes plain text (anywhere in the path), /pattern/ for a regex, ~text for a fuzzy match and *.ts or src/** for a glob, and shows matching / all file counts while active.",
              },
              {
                kind: "paragraph",
            ...
            blocks: [
              {
                ...
                rows: [
                  ...
                  [
                    "view-state.json",
                    "File list tree state — collapsed directories, lazy-expanded directories (folders opened on demand in large repos), and the viewed-file list used to dim already-read entries.",
                  ],
                  [
                    "tabs.json",
            ...
            blocks: [
              {
                ...
                rows: [
                  ...
                  [
                    "Sidebar",
                    "DB selector, PostgreSQL schema selector, table tree (expand to see columns and a table description when available), filter, Rails FK inference toggle, and icon toolbar for ER / Search / Snapshot tabs.",
                  ],
                  [
                    "Data tab",
                  ...
                  [
                    "Query editor",
                    "Sits above the result grid on the same screen: drag the divider (or focus it and press ↑ / ↓) to change its height, or collapse it. Choosing a table shows its rows below and puts the SQL that fetched them (with the page's LIMIT / OFFSET) into the editor; Run and Explain show their results in the same place, and the Data tab goes back to the table. SQL syntax highlighting (shiki), Tab indent, Ctrl+Enter to run. Allowlist depends on the engine (SQLite: SELECT/PRAGMA/EXPLAIN/WITH; PostgreSQL and MySQL also accept SHOW/DESCRIBE).",
                  ],
                  [
                    "ER Diagram",
        ...
        groups: [
          ...
          },
          {
            ...
            blocks: [
              ...
              {
                kind: "paragraph",
                text: "ファイル一覧やファイルパレットから、ソース、Markdown プレビュー、画像、PDF などを開けます。大きいテキストファイルは自動で軽量な仮想表示に切り替わります。ファイル一覧の行はリンクなので、Cmd/Ctrl+クリックや中クリックで別タブに開けます。diff カードの View File はファイル一覧（履歴画面ではコミット一覧も）を残したままファイル全体を表示し、View Diff で差分に戻ります。",
              },
              {
                kind: "paragraph",
                text: "ファイル詳細ページには最大 4 つのタブ (Preview / Code / Blame / History) があります。テキストファイルは Code がデフォルトで、?preview=1 を付けると Markdown / HTML プレビューに切り替わります。画像・動画・音声・PDF などのメディアファイルは Preview タブのみ表示され、Code タブは表示されません。Blame と History はそれぞれ専用 URL (view=blame, view=history) を持つため、ディープリンクとブラウザの戻る / 進むが同期し、いずれもファイル一覧は表示されたままです。ツリーから別のファイルを開いても選択中のタブは維持されます (プレビューできないファイルでは Code に戻ります)。ファイルの見出しが狭いとき (2 面の右の面など) は、このタブを 2 段目に下ろします。幅が約 510px より狭いと、パンくずが 1 段目を全部使い、その横の操作 (パスのコピー・OS で開く・情報・前後・削除) は 2 段目に並びます。",
              },
              {
                kind: "paragraph",
              ...
              {
                kind: "paragraph",
                text: "大きいリポジトリではファイル一覧がフォルダの中身を必要に応じて読み込みます。開いたフォルダは記憶され、次回のリロード時に同じ状態で展開し直されます。",
              },
              {
                kind: "paragraph",
              ...
              {
                kind: "paragraph",
                text: "木には Tab で 1 回だけ止まります (選んでいる行、無ければ先頭の行)。そこで ↑ / ↓ で移り、Home / End で先頭 / 末尾へ、→ でフォルダを開き、← で畳むか親のフォルダへ移り、Enter で開きます。j / k / l / h はこれまでどおり使えます。Shift+L で本文へフォーカスを移し、Shift+H でファイル一覧 (Diff・History・作業ツリーではその画面の一覧) へ戻ります。",
              },
              {
                kind: "paragraph",
          ...
          },
          {
            ...
            blocks: [
              ...
              {
                kind: "paragraph",
                text: "ファイル一覧の絞り込みは文字列（パスの部分一致）、/pattern/（正規表現）、~text（あいまい一致）、*.ts や src/**（glob）を受け付け、絞り込み中は「一致 / 全体」の件数を表示します。",
              },
              {
                kind: "paragraph",
          ...
          },
          {
            ...
            blocks: [
              ...
              {
                kind: "paragraph",
                text: "打ったキーはそのシェルに届くので、タブを離れずに返事ができます。見るだけにしたいときはタブの右クリックで入力を切れます (閲覧のみ)。同じメニューで文字の大きさも変えられます (どちらも全部のターミナルのタブに効きます)。ターミナルのタブの名前は映しているもので、エージェントならその種類と状態、ただのシェルなら「シェル」と番号です。シェルはサーバが動いている間だけ生き、exit と打てばタブごと閉じます (「セッションを止める」でも同じです)。tmux のペインを開いたタブは、そのペインのある tmux のウインドウを映しています。ペインが終わっても同じウインドウに別のペインが残っていれば、タブは閉じず、tmux が前に出したペインを映し続けます (タブの名前もそのペインに変わります)。ウインドウが終わると閉じます (セッションの別のウインドウへは移りません)。tmux から抜けた (セッションが終わった・detach した) ときも閉じます。何が終わったかは右下に短く出ます。同じ tmux のセッションをもっと小さい端末でも開いていると、tmux はウインドウをその大きさに縮め、余りを点で埋めます。ターミナルのタブはその余りを覆い、ウインドウの大きさと理由を出します。",
              },
              {
                kind: "paragraph",
            ...
            blocks: [
              {
                ...
                rows: [
                  ...
                  [
                    "view-state.json",
                    "ファイル一覧のツリーの状態 — 折りたたみ済みディレクトリ、遅延展開済みディレクトリ（大規模リポジトリで必要に応じて開かれたフォルダ）、既読扱いするための表示済みファイル一覧。",
                  ],
                  [
                    "tabs.json",
            ...
            blocks: [
              {
                ...
                rows: [
                  ...
                  [
                    "サイドバー",
                    "DB 選択、PostgreSQL スキーマセレクター、テーブルツリー（展開でカラムと、あればテーブルコメントを表示）、フィルター、Rails 命名規約による仮想 FK 推測トグル、ER / Search / Snapshot アイコンツールバー。",
                  ],
                  [
                    "Data タブ",
                  ...
                  [
                    "クエリエディター",
                    "結果の表の上に同じ画面で置かれます。区切りをドラッグして (区切りにフォーカスして ↑ / ↓ でも) 高さを変えたり、畳んだりできます。表を選ぶと下にその行が出て、入力欄には取得に使った SQL (そのページの LIMIT / OFFSET 入り) が入ります。Run と Explain の結果も同じ場所に出て、Data タブで表に戻ります。SQL シンタックスハイライト (shiki)、Tab インデント、Ctrl+Enter で実行。許可文は DB 種別により異なります（SQLite: SELECT/PRAGMA/EXPLAIN/WITH。PostgreSQL と MySQL は SHOW/DESCRIBE も可）。",
                  ],
                  [
                    "ER 図",
