        ...
        groups: [
          ...
          },
          {
            ...
            blocks: [
              ...
              {
                kind: "paragraph",
                text: "最上段のタブ列に、開いたファイルと画面が並びます (その上に見出しの行はありません)。プロジェクト名・ブランチと 6 つの画面の絵柄 (Files・Diff・History・Worktrees・Data・Work log。カーソルを置くと名前とキー) は右の列の頭 (タブ列の行とその下) に固定されます。右の列には Files の木もあり、左のサイドバーにはプロジェクトとエージェントが並びます。本文を選ぶための一覧 (Diff の変更ファイル・History のコミット・選んでいる作業ツリーの一覧) は、左のサイドバーと本文の間の一覧の列に出ます。その画面の間は右の列を細い帯に畳み、帯のボタンは一覧の列を隠す / 出すボタンになります。一覧の列はドラッグした幅を覚え (最初は 320px)、本文が 480px に足りないときは 240px に詰めます (History は件名と枝の札だけになります)。History の枝の札は、件名と分け合う幅の約 40% まで (件名が短ければそれ以上) 名前を省略せずに出し、件名を残りの幅に収まるよう省略します。別のプロジェクトのエージェントを映すターミナルのタブは「プロジェクト名 · 題」になり、タブが狭いときはプロジェクト名から省略します。タブや画面を切り替えても動かず、右の列を畳んだときだけタブ列の左端に移ります。その頭が狭いときは、ブランチの名前を幅の約 40% まで (プロジェクト名が短ければそれ以上) 省略せずに出し、プロジェクト名を残りの幅に収まるよう省略します。1 回押して開いたファイル (木・パレット・行リンク・View File) は名前が斜体の仮のタブで、次に開いたファイルに置き換わります。木の行かタブをダブルクリックするか、「開いたままにする」で固定します。置き換わらない自分のタブで開くには、中ボタンか ⌘/Ctrl+クリック (木・Diff と History のファイルの一覧・Search の結果・パレットの行)、パレットで Shift+Enter、木のファイルの右クリックの「新しいタブで開く」を使います。そのファイルが開いていれば、そのタブを前面に出して固定します。Shift+クリックは今までどおりブラウザの新しいウィンドウで開きます。Diff・History・Worktrees・Data・Work log はそれぞれタブが 1 つで、絵柄を押すとそのタブが前に見ていた状態で前面に出ます。Files はタブではありません。フォルダ表示は、左の面でタブを選んでいないときに出る既定の本文です (Files の絵柄・g r・木のフォルダで選択が外れます。最後のタブを閉じたときもこれが出ます)。タブの右クリック (タブの上で Shift+F10 でも) で閉じる・ほかを閉じる・右側を閉じる・開いたままにする・パスをコピー、ドラッグで並べ替え、g t / g T (次 / 前)、g x (閉じる)、g 1〜9 で移れます。タブ列の上では ←/→・Home・End でタブを移り、Enter で前面に、Delete で閉じ、Ctrl+Shift+PageUp / PageDown で並べ替えます。タブが列に入りきらないときは、1 つずつ 88px まで縮めて名前を省略します (絵と閉じるは残ります)。それでも入らなければ列を横に送り、前面のタブは必ず見える位置に出します。タブの並びはプロジェクトごとに覚えます。ただしプロジェクトに属さないタブ (エージェントの全体ボード・Tools・設定とヘルプ・ターミナル・ターミナルに出た画像) は、プロジェクトを切り替えても開いたまま残ります。画面 (Diff・History・Worktrees・Data・Work log・エージェント・設定とヘルプ) は描く場所が 1 つなので左の面にだけ置き、右の面にはファイル・ターミナル・画像を置きます。ファイルは面ごとに描くので、2 つのファイルや同じファイルを左右に並べられ、スクロール・行の選択・Code / Preview / Blame の切り替えは面ごとです (ファイルの History は左の面で開きます)。タブ列の右端の分割ボタン (ファイル・ターミナル・画像のタブの「右に分割」、それを右半分の破線の枠へドラッグでも) で左右 2 面になり (押せないときはボタンの説明に理由が出ます)、間の線をドラッグで幅を変え、ファイル・ターミナル・画像は面の間でドラッグするか「反対側へ移す」で移し、g o でもう一方の面へ移ります。木とパレットはフォーカスのある面にファイルを開き、木・Diff と History のファイルの一覧・Search の結果を Alt+クリック (木のファイルの右クリックの「右に分割して開く」でも) すると反対の面に開きます (1 面なら右に分けて右に開きます)。URL はフォーカスのある面に合わせ、右の面のファイルなら pane=right が付くので、再読み込みや戻る・進むでその面に戻ります。画像のファイルは、倍率・前後・パスのコピー・フォルダを開くが付いた画像のタブで開きます。以前の見出しの行の右端にあったボタン (注釈・AI 用コンテキストのコピー・自動更新・通信の中止・テーマ・リポジトリのページ) は最下段の右にあります。",
              },
              {
                kind: "paragraph",
                text: "右の列を開いたままだと左右の面がそれぞれ 480px に足りないときは、2 面の間だけ右の列を細い帯に畳みます (帯のボタンに印と理由が出ます)。1 面に戻すと開き直します。2 面の間に自分で開いたときは、再読み込みまで開いたままにします。Diff・History・選んでいる作業ツリーでは右の列はもう帯です。左右の面がそれぞれ 480px に足りないときは、まず一覧の列を 240px に詰め、それでも 2 面が入らなければ、別の画面へ移るか窓を広げるまで右の面を預けます (分割のボタンにそう出ます)。",
              },
              {
                kind: "paragraph",
