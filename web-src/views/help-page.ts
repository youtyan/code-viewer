        ...
        groups: [
          ...
          },
          {
            ...
            blocks: [
              {
                kind: "paragraph",
                text: "左のサイドバーは、どの画面でもプロジェクトとその中で動いているエージェントを並べます。上に登録したプロジェクト (好きな順)、その下の「tmux で検出」に、登録していないが tmux でエージェントが動いているプロジェクトが出ます。状態が変わっても行は動きません (変化は状態の印・最下段の件数・通知で分かります)。プロジェクトの中のエージェントは tmux の場所の順です。プロジェクト名を押すとそのプロジェクトへ移り (登録していなければ確かめずに登録してから)、左の山形で畳み、エージェントを押すとメインの面のターミナルのタブにそのペインが開きます。Alt+クリック、または右クリックの「反対の面で開く」は反対側の面を使い、1 面だけなら左右に分割して右に開きます。「すべてのエージェント」のボード (「プロジェクト」の横のボタン、g a) は、このマシンの tmux でコーディングエージェントが動いているペインを、プロジェクトごとに並べます。プロジェクトはペインのフォルダが属する git リポジトリで、作業ツリーは本体にまとめ、git 管理外のフォルダはそのフォルダで 1 つにします。どのリポジトリで開いた code-viewer からでも同じ一覧です。エージェントはサイドバーと同じ 2 行組のカードです。1 行目は作業内容 (ペインのタイトル。無ければ種類: claude、codex、フックで状態を申告するエージェント) で、見ていない間に入力待ちになった・終わったものには札が付きます。2 行目に種類・状態・その状態になってからの時間・作業ツリー・アカウント・tmux 上の場所 (セッション:ウィンドウ.ペイン) が並びます。サイドバーでは、プロジェクトはエージェントの数を添えた太い見出しで、その下にエージェントのカードが字下げして並びます。入力待ちが先頭、次に完了・未読、作業中、その後にそれ以外。Enter かクリックで、メインの面のターミナルのタブにそのペインが開きます。ただのシェルは「すべてのペイン」にしたときだけ出ます。",
              },
              {
                kind: "paragraph",
