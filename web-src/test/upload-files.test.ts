...
describe("upload files endpoint security", () => {
  test("upload is explicitly opt-in and uses a side-effect request gate", () => {
    ...
    expect(
      server.includes("function loadProjectConfigUploadEnabled(): boolean"),
    ).toBe(true);
    ...
    expect(
      server.includes("if (!allowUpload) return text('upload disabled', 403)"),
    ).toBe(true);
    expect(
      server.includes(
  ...
  test("upload accepts only bounded multipart form data", () => {
    expect(server.includes("return text('method not allowed', 405)")).toBe(
      true,
    );
    expect(server.includes("multipart\\/form-data")).toBe(true);
    expect(server.includes("MAX_UPLOAD_BODY_BYTES")).toBe(true);
    expect(server.includes("return text('content length required', 411)")).toBe(
      true,
    );
    expect(server.includes("return text('invalid content length', 400)")).toBe(
      true,
    );
  ...
  test("upload validates directory and file names before writing", () => {
    ...
    expect(
      server.includes("function isForbiddenUploadName(name: string): boolean"),
    ).toBe(true);
    expect(
      server.includes("'.png', '.jpg', '.jpeg', '.gif', '.webp', '.pdf'"),
    ).toBe(true);
    ...
    expect(
      server.includes(
        "/^(tsconfig|jsconfig|bunfig|vercel|netlify|wrangler|next|vite|webpack|rollup|esbuild|astro|svelte|tailwind|postcss|babel|prettier|eslint)\\./",
      ),
    ).toBe(true);
...
describe("folder upload UI", () => {
  test("repository folder view exposes a drop zone and upload button", () => {
    expect(
      app.includes("function createRepoUploadPanel(path: string): HTMLElement"),
    ).toBe(true);
