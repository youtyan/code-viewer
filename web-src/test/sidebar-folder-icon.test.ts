...
const appSource = sourceFixture(
  readFileSync("web-src/app.ts", "utf8") +
    readFileSync("web-src/core/icons.ts", "utf8") +
...
describe("sidebar folder icons", () => {
  ...
  test("marks intentionally omitted directory children in the sidebar", () => {
    ...
    expect(appSource.includes("li.classList.add('children-omitted')")).toBe(
      true,
    );
    expect(
      appSource.includes(
    ...
    expect(appSource.includes("children_omitted: entry.children_omitted")).toBe(
      true,
    );
    expect(
      appSource.includes(
    ...
    expect(style.includes("#filelist.tree .tree-dir.children-omitted")).toBe(
      true,
    );
    expect(
      style.includes("#filelist.tree .tree-dir.children-omitted-heavy"),
    ).toBe(true);
    expect(
      style.includes("#filelist.tree .tree-dir.children-omitted-internal"),
    ).toBe(true);
    expect(style.includes("#filelist.tree .tree-dir .dir-omitted")).toBe(true);
    expect(style.includes("#filelist.tree .tree-dir .dir-omitted-heavy")).toBe(
      true,
    );
    expect(
      style.includes(
