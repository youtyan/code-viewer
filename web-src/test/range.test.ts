...
describe('parseHttpByteRange', () => {
  test('parses a bounded byte range', () => {
    expect(parseHttpByteRange('bytes=2-5', 10)).toEqual({ kind: 'range', range: { start: 2, end: 5 } });
  });

  test('parses a suffix byte range', () => {
    expect(parseHttpByteRange('bytes=-4', 10)).toEqual({ kind: 'range', range: { start: 6, end: 9 } });
  });

  test('parses an open-ended byte range', () => {
    expect(parseHttpByteRange('bytes=2-', 10)).toEqual({ kind: 'range', range: { start: 2, end: 9 } });
  });

  test('rejects unsatisfiable or malformed byte ranges', () => {
    expect(parseHttpByteRange('bytes=10-12', 10)).toEqual({ kind: 'unsatisfiable' });
    expect(parseHttpByteRange('bytes=-0', 10)).toEqual({ kind: 'unsatisfiable' });
    expect(parseHttpByteRange('items=0-1', 10)).toEqual({ kind: 'invalid' });
    expect(parseHttpByteRange('bytes=0-1,2-3', 10)).toEqual({ kind: 'invalid' });
    expect(parseHttpByteRange('bytes=5-2', 10)).toEqual({ kind: 'unsatisfiable' });
  });
});

describe('collectLineRangeFromStream', () => {
  test('collects only the requested 1-based line range from a stream', async () => {
    const result = await collectLineRangeFromStream(streamFromText('one\ntwo\nthree\nfour\n'), 2, 3);
    expect(result).toEqual({ lines: ['two', 'three'], total: 4, complete: false });
  });

  test('handles CRLF and files without a final newline', async () => {
    const result = await collectLineRangeFromStream(streamFromText('one\r\ntwo\r\nthree'), 2, 5);
    expect(result).toEqual({ lines: ['two', 'three'], total: 3, complete: true });
  });
  ...
  test('distinguishes exact EOF from a following line after the requested range', async () => {
    expect(await collectLineRangeFromStream(streamFromText('one\ntwo\nthree'), 1, 3))
...
describe('collectByteRangeFromStream', () => {
  test('collects a byte slice across stream chunks', async () => {
    const bytes = await collectByteRangeFromStream(streamFromText('zero\none\ntwo\nthree'), 5, 12);
...
describe('line offset index', () => {
  test('maps a 1-based line range to the minimal byte slice', () => {
    ...
    expect(lineByteRangeForIndex(index, 3, 3)).toEqual({ start: 8, endExclusive: 13 });
  });

  test('builds the same line offset index from a stream without requiring a single byte array', async () => {
    const text = 'one\ntwo\nthree\nfour';
    const bytes = new TextEncoder().encode(text);
    const index = await buildLineOffsetIndexFromStream(streamFromText(text), bytes.length);
  ...
  test('uses the actual stream byte count when it differs from the hinted size', async () => {
    const text = 'one\ntwo';
    const index = await buildLineOffsetIndexFromStream(streamFromText(text), 999);
  ...
  test('collects bytes and a line offset index from a stream in one pass', async () => {
    const text = 'one\ntwo\nthree';
    const result = await collectBytesWithLineOffsetIndexFromStream(streamFromText(text), 0);
    expect(new TextDecoder().decode(result.bytes)).toBe(text);
    expect(result.index).toEqual(buildLineOffsetIndex(new TextEncoder().encode(text)));
  });
  ...
  test('collects indexed lines with the same total and complete semantics as streams', async () => {
    ...
    const indexed = collectLineRangeFromIndexedText(text.slice(range!.start, range!.endExclusive), index, 2, 3);
    expect(indexed).toEqual({ lines: ['two', 'three'], total: 4, complete: false });
    expect(indexed).toEqual(await collectLineRangeFromStream(streamFromText(text), 2, 3));
  });
  ...
  test('returns empty complete indexed results past EOF', () => {
    ...
    expect(collectLineRangeFromIndexedText('', index, 10, 12))
      .toEqual({ lines: [], total: 3, complete: true });
  });
  ...
  test('keeps empty lines when an indexed byte slice has zero length', async () => {
    ...
    const indexed = collectLineRangeFromIndexedText(text.slice(range.start, range.endExclusive), index, 1, 1);
    expect(indexed).toEqual(await collectLineRangeFromStream(streamFromText(text), 1, 1));
  });

  test('sets indexed complete exactly at EOF boundaries', () => {
    const text = 'one\ntwo\nthree';
    const index = buildLineOffsetIndex(new TextEncoder().encode(text));
    expect(collectLineRangeFromIndexedText('one\ntwo', index, 1, 2).complete).toBe(false);
    expect(collectLineRangeFromIndexedText('one\ntwo\nthree', index, 1, 3).complete).toBe(true);
  });
});
