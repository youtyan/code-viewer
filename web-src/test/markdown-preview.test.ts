...
const markdown = readFileSync(new URL('../markdown-preview.ts', import.meta.url), 'utf8');
const server = readFileSync(new URL('../server/preview.ts', import.meta.url), 'utf8');
...
describe('markdown preview', () => {
  test('uses markdown-it with raw HTML disabled and repository-aware overrides', () => {
    ...
    expect(markdown.includes("buildRawFileUrl({ path: resolved, ref: target.ref || 'worktree' })")).toBe(true);
  ...
  test('resolves relative markdown links inside the repository', () => {
    expect(resolveMarkdownRelativePath('docs/guide/intro.md', './next.md')).toBe('docs/guide/next.md');
    expect(resolveMarkdownRelativePath('docs/guide/intro.md', '../README.markdown#top')).toBe('docs/README.markdown');
    expect(resolveMarkdownRelativePath('docs/guide/intro.md', 'https://example.com/a.md')).toBe(null);
    expect(resolveMarkdownRelativePath('docs/guide/intro.md', './image.png')).toBe(null);
    expect(resolveMarkdownRelativePath('docs/guide/intro.md', '../../../README.md')).toBe(null);
  });

  test('resolves relative markdown image assets through raw file URLs', () => {
    expect(resolveMarkdownAssetPath('docs/guide/intro.md', './img/screen.png')).toBe('docs/guide/img/screen.png');
    expect(resolveMarkdownAssetPath('docs/guide/intro.md', '../assets/logo.svg?raw=1')).toBe('docs/assets/logo.svg');
    expect(resolveMarkdownAssetPath('docs/guide/intro.md', '/img/logo.svg')).toBe('img/logo.svg');
    expect(resolveMarkdownAssetPath('docs/guide/intro.md', 'https://example.com/logo.png')).toBe(null);
    expect(resolveMarkdownAssetPath('docs/guide/intro.md', '../../../etc/passwd')).toBe(null);
  });

  test('renders task lists as list items that can be enhanced after parsing', () => {
    const html = renderMarkdownHtml('- [x] done\n- [ ] todo\n', { path: 'README.md', ref: 'worktree' }, null);
  ...
  test('renders highlighted code blocks with Shiki markup', () => {
    const highlighter = {
      codeToHtml: (code: string, options: { lang: string }) =>
        '<pre class="shiki"><code><span class="line" data-lang="' + options.lang + '">' + code + '</span></code></pre>',
    };
    const html = renderMarkdownHtml('```ts\nconst value = 1;\n```', { path: 'README.md', ref: 'worktree' }, highlighter);
  ...
  test('normalizes common fenced language aliases before Shiki rendering', () => {
    ...
    const html = renderMarkdownHtml('```sh\necho hello\n```', { path: 'README.md', ref: 'worktree' }, highlighter);
  ...
  test('normalizes TypeScript and JSX-style fenced language aliases', () => {
    ...
    renderMarkdownHtml('```tsx\nreturn value;\n```', { path: 'README.md', ref: 'worktree' }, highlighter);
    renderMarkdownHtml('```jsx\nreturn value;\n```', { path: 'README.md', ref: 'worktree' }, highlighter);
  ...
  test('mermaid is built as a lazy standalone asset and served by the preview server', () => {
    ...
    expect(server.includes("'/mermaid.js': ['mermaid.js', 'application/javascript; charset=utf-8']")).toBe(true);
  ...
  test('Shiki is built as a lazy standalone asset for markdown code blocks', () => {
    ...
    expect(server.includes("'/shiki.js': ['shiki.js', 'application/javascript; charset=utf-8']")).toBe(true);
  ...
  test('markdown preview applies Shiki light and dark theme variables', () => {
    ...
    expect(style.includes('[data-theme="dark"] .gdp-markdown-preview pre.shiki span')).toBe(true);
  });
});
