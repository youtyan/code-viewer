...
describe('mark viewed toolbar button', () => {
  ...
  test('persists per-file viewed paths without dimming diff bodies', () => {
    ...
    expect(appSource.includes("li.classList.toggle('viewed', !onFileClick && STATE.viewedFiles.has(f.path))")).toBe(true);
    expect(appSource.includes("li.classList.toggle('viewed', !isRepositorySidebarMode() && STATE.viewedFiles.has(path))")).toBe(true);
    expect(appSource.includes('if (isRepositorySidebarMode()) return')).toBe(true);
    expect(appSource.includes('function syncViewedCardDisplay(card: HTMLElement, viewed: boolean)')).toBe(true);
    expect(appSource.includes('function applyViewedToCard(card: HTMLElement, viewed: boolean, collapseLoaded = false)')).toBe(true);
    expect(appSource.includes('setFileCollapsed(card as DiffCardElement, viewed)')).toBe(true);
    expect(appSource.includes('syncViewedCardDisplay(card, viewed)')).toBe(true);
    expect(appSource.includes('applyViewedToCard(card, viewed, true)')).toBe(true);
    expect(appSource.includes('applyViewedToCard(card, STATE.viewedFiles.has(file.path), true)')).toBe(true);
