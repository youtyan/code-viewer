...
describe("decideAttachedPane", () => {
  test.each([
    {
      name: "映しているペインのまま",
      client: { session: "work", pane: "%1" },
      exists: null,
      expected: { kind: "keep" },
    },
    {
      name: "ペインが終わり、tmux が同じセッションの別のペインへ移した",
      client: { session: "work", pane: "%2" },
      exists: false,
      expected: { kind: "close" },
    },
    {
      name: "ペインが終わり、tmux が別のセッションへ移した (detach-on-destroy off)",
      client: { session: "other", pane: "%9" },
      exists: false,
      expected: { kind: "close" },
    },
    {
  ...
  ] as const)("$name", ({ client, exists, expected }) => {
    expect(decideAttachedPane(WATCHED, client, exists)).toEqual(expected);
  });
});
...
describe("watchAttachedShell", () => {
  ...
  test("tmux が同じセッションの別のペインへ移したら、シェルを閉じて見張りをやめる", async () => {
