...
describe('diff catch-up policy', () => {
  ...
  test('runs for diff and file detail routes only', () => {
    ...
    expect(shouldCatchUpDiff({ screen: 'file', path: 'README.md', ref: 'worktree', view: 'detail', range })).toBe(true);
    expect(shouldCatchUpDiff({ screen: 'repo', ref: 'worktree', path: '', range })).toBe(false);
    expect(shouldCatchUpDiff({ screen: 'file', path: 'README.md', ref: 'worktree', view: 'blob', range })).toBe(false);
  });
