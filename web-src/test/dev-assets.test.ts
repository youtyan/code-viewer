...
describe('dev asset reload', () => {
  test('does not watch assets outside dev mode', () => {
    ...
    const started = startDevAssetReload({
      ...
      webRoot: '/repo/web',
      watchedFiles: ['app.js'],
      watch: (() => { watched = true; }) as WatchFn,
      sendReload: () => { throw new Error('reload should not run'); },
    });
  ...
  test('debounces reload events for watched asset changes', () => {
    ...
    const started = startDevAssetReload({
      ...
      watch: ((_path, _options, next) => { listener = next; }) as WatchFn,
      sendReload: () => { reloads++; },
      setTimeoutFn: ((callback: () => void) => { scheduled = callback; return 1; }) as typeof setTimeout,
      clearTimeoutFn: (() => { cleared++; }) as typeof clearTimeout,
    });
