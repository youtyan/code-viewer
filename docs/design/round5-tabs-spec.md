...
- 実装追記: Round 5 後の UI 修正で `redis-explorer.ts` /
  ...
  docker utility 共通化で変更済み。Phase 11 では `snapshot-runner.ts` に
  cancel signal を通し、`adapters/docker.ts` も docker utility 共通化へ移行済み。
  現行の保護対象は `docs/design/post-round-deviations.md` を参照。
