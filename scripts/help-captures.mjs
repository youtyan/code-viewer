...
async function captureLanguage(options, chrome, lang, text) {
  ...
  try {
    ...
    // 止まるまで待つ。待たずに次の言語の砂場を消すと、止まる途中のサーバが砂場に
    // 書いていて消しきれない (ENOTEMPTY) ことがあった。
    await stopServer(server);
...
async function main() {
  ...
  // 端末の文字は DOM の行 (.xterm-rows) から読んで待つので、WebGL を切って
  // DOM の描画にする (WebGL が使えない環境と同じ、アプリの正規の代替の経路)。
  const chrome = options.setupOnly
    ? null
    : await startChrome(["--disable-webgl"]);
  const killChrome = () => {
